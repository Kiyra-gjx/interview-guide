...
export default function AnalysisPanel({
  ...
  const radarData = useMemo(() => {
    ...
    return [
      ...
      { subject: '技能匹配度', score: skillMatchScore, fullMark: 20 },
  ...
  const errorDisplay = useMemo(
    () => getAnalyzeErrorDisplay(analyzeErrorCode, analyzeError),
    [analyzeError, analyzeErrorCode],
  );
  ...
  const hasTechnicalErrorKeywords = typeof analysis?.summary === 'string' && (
    analysis.summary.includes('I/O error') ||
    analysis.summary.includes('Exception') ||
    analysis.summary.includes('Remote host terminated') ||
    analysis.summary.includes('handshake') ||
    analysis.summary.includes('software.amazon') ||
    analysis.summary.includes('com.fasterxml')
  );

  const isAnalysisValid = Boolean(analysis && analysis.summary && !hasTechnicalErrorKeywords);
  const isProcessing = analyzeStatus === 'PENDING'
    || analyzeStatus === 'PROCESSING'
    || (analyzeStatus === undefined && !analysis);
  const canRetry = Boolean(onReanalyze) && analyzeRetryable !== false;
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-12 text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-4">
          {isExplicitProcessing
            ...
            : '简历已上传成功，系统即将开始 AI 分析。'}
        </p>
        <p className="text-sm text-slate-400 dark:text-slate-500">页面会自动刷新展示分析结果。</p>
      </div>
    );
  }
  ...
  if (analyzeStatus === 'FAILED' || !isAnalysisValid) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">{errorDisplay.title}</h3>
        <p className="text-slate-500 dark:text-slate-400 mb-4">{errorDisplay.message}</p>
        <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg text-left mb-4">
          <p className="text-sm text-red-600 dark:text-red-400">{errorDisplay.hint}</p>
        </div>
        {canRetry && onReanalyze && (
          <motion.button
            onClick={onReanalyze}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="bg-gradient-to-br from-emerald-50 dark:from-emerald-900/30 to-green-50 dark:to-slate-800 rounded-xl p-6">
            <p className="text-lg text-slate-800 dark:text-white leading-relaxed mb-6">
              {analysis.summary || '候选人具备扎实的技术基础，并有较完整的项目经验。'}
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 block mb-2">分析时间</span>
            ...
            {analysis.strengths && analysis.strengths.length > 0 && (
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 block mb-3">优势亮点</span>
        ...
        >
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <ScoreProgressBar label="项目经验" score={projectScore} maxScore={40} color="bg-purple-500" delay={0.3} className="col-span-2" />
            <ScoreProgressBar label="技能匹配度" score={skillMatchScore} maxScore={20} color="bg-blue-500" delay={0.4} />
            ...
            <ScoreProgressBar label="表达专业性" score={expressionScore} maxScore={10} color="bg-orange-500" delay={0.7} />
          </div>
        </motion.div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-6">
          ...
          <span className="text-sm text-slate-400 dark:text-slate-500">({analysis.suggestions?.length || 0} 条)</span>
        </div>

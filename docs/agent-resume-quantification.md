...
## 0. 截至 2026-04-28 的当前真实结果

当前仓库里已经有 5 组可复核证据，不再是纯规划态：

- `stage-3-context-set`：`10/10` case 通过，平均上下文长度 `347.3 -> 256.1`，平均压缩率 `16.09%`，最高压缩率 `63.53%`，关键 section 保留率 `100%`
- `stage-3-memory-set`：`9/9` case 通过，`repeatedToolCalls 3 -> 0`，`repeatedFactChecks 2 -> 0`，`extraCallsAfterMemoryReady=0`
- `stage-5-recovery-set`：`9/9` case 通过，恢复正确率 `100%`，`wrongStateContinued=0`，`replayedSideEffect=0`
- `stage-2-safety-set`：`10/10` case 通过，高风险动作审批命中率 `100%`，审批拒绝后降级收口率 `100%`，`directExecutionBypassed=0`
- `stage-5-benchmark`：`4/4` case 通过，覆盖只读委派成功、单步拒绝委派、步数预算耗尽和审批恢复阻断重放

基于这 5 组证据，现在已经能稳定写出 4 类简历量化句子：
...
如果你现在只是为了写简历，而不是继续补新能力，顺序已经可以改成“先收口、再写句子”：

1. 先从 `docs/evidence/agent-quantification/` 回收当前已完成的 5 组证据
2. 再从每组 `summary.md` 挑出 resume-safe 句子
3. 最后再决定保留哪 3 到 4 条进简历，不要把所有指标全塞进去
...
## 10.1 当前这版项目已经可直接使用的句子类型

下面这些口径已经有真实证据支撑：

- `Context`：
  在 `10` 组固定上下文配置中，将平均上下文长度从 `347.3` 压到 `256.1`，平均压缩率 `16.09%`，关键 section 保留率 `100%`，未出现 `requestBroken`
- `Memory`：
  围绕 memory phase、facts 归一化、委派写回和 follow-up 复用构建 `9` 组固定样本，将重复工具读取从 `3` 次降到 `0`，重复事实确认从 `2` 次降到 `0`
- `Recovery`：
...
1. 如果要继续补更强证据，按第 4 节和第 5 节继续扩样本
2. 如果要写当前版本简历，直接从第 10.1 节和 `docs/agent-resume-project-draft.md` 取句子
3. 每次新增采数后，仍要先更新 evidence summary，再更新简历初稿
4. 写最终简历时，优先保留 3 到 4 条最强 bullet，不要把所有指标堆进去

当前已经完成 Context / Memory / Recovery / Safety 四类高质量证据，可以进入简历初稿阶段。
